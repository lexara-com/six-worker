...
-- Set the node ID we're examining
\set target_node_id '01K6GMKE4Z0GHW7637XAB9KYVR'
...
WHERE r.source_node_id = :'target_node_id'
  AND r.status = 'active'
...
WHERE r.target_node_id = :'target_node_id'
  AND r.status = 'active'
ORDER BY r.relationship_type, r.created_at;
...
WITH potential_duplicates AS (
    SELECT 
        ...
        COUNT(*) as count
    FROM relationships
    WHERE (source_node_id = :'target_node_id' OR target_node_id = :'target_node_id')
      AND status = 'active'
...
SELECT 
    CASE 
        WHEN source_node_id = :'target_node_id' THEN 'Outgoing'
        ELSE 'Incoming'
    END as direction,
    relationship_type,
    count,
...
WITH connected_nodes AS (
    ...
    FROM relationships
    WHERE source_node_id = :'target_node_id' AND status = 'active'
    
    UNION
    
    SELECT DISTINCT source_node_id as connected_node_id
    FROM relationships
    WHERE target_node_id = :'target_node_id' AND status = 'active'
)
SELECT 
    n.node_type,
    COUNT(*) as count

-- =============================================
-- Clean Up Function Signatures (Explicit)
-- Version: 9.0
-- Purpose: Remove duplicate functions with explicit signatures
-- =============================================

-- Drop specific function signatures
DROP FUNCTION IF EXISTS create_provenance_record(VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, NUMERIC);
DROP FUNCTION IF EXISTS create_provenance_record(VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR, NUMERIC, VARCHAR, JSONB);
...
SELECT status, overall_confidence FROM propose_fact(
...
\echo '2. Check if any functions still conflict:'
SELECT proname, pg_get_function_arguments(oid)
FROM pg_proc 
WHERE proname = 'create_provenance_record';
...
-- Version: 4.0
-- Purpose: Accept proposed facts (2 nodes + 1 relationship) and intelligently 
--          resolve entities, score relationships, manage provenance
-- =============================================

-- Enable trigram extension for fuzzy string matching (if not already enabled)
...
BEGIN
    ...
    -- Strategy 3: Fuzzy name matching (simplified - in production would use more sophisticated algorithms)
...
BEGIN
    ...
    -- Find conflicting relationships (e.g., Legal_Counsel vs Opposing_Counsel)
    FOR rel_record IN
        SELECT r.relationship_id, r.relationship_type, r.strength, r.status
        FROM relationships r
        WHERE ((r.source_node_id = p_source_id AND r.target_node_id = p_target_id) OR
...
BEGIN
    ...
    IF exact_match IS NOT NULL THEN
        -- Relationship already exists - evaluate if we should update

-- =============================================
-- V13: Fix Address Deduplication
-- =============================================
-- Problem: The propose_geographic_fact function always creates new address nodes
--          without checking for existing ones, causing massive duplication
--
-- Solution: Update the function to check for existing addresses before creating new ones
-- =============================================

-- Drop the existing function to replace it
DROP FUNCTION IF EXISTS propose_geographic_fact;

-- Create improved version that checks for existing addresses
...
BEGIN
    ...
    IF v_location_id IS NULL THEN
        ...
        INSERT INTO nodes (node_id, node_type, primary_name, normalized_name, entity_class, created_by)
        VALUES (v_location_id, p_location_type, INITCAP(p_location_name), v_normalized_location, 'fact_based', 'geo_system');
    ...
    IF v_entity_id IS NULL THEN
        ...
        VALUES (v_entity_id, p_entity_type, p_entity_name, normalize_name(p_entity_name), 'geo_system');
    END IF;
    
    -- FIX: Check for existing address before creating new one
    IF p_address IS NOT NULL AND p_address != '' THEN
        ...
        IF v_address_id IS NULL THEN
            ...
            INSERT INTO nodes (node_id, node_type, primary_name, normalized_name, entity_class, created_by)
            VALUES (v_address_id, 'Address', p_address, v_normalized_address, 'fact_based', 'geo_system');
            ...
            IF p_coordinates IS NOT NULL THEN
                ...
                VALUES (v_address_id, 'coordinates', p_coordinates::TEXT, 'geo_system');
            END IF;
        END IF;
...
-- Test the fix with a known duplicate address
DO $$
DECLARE
    v_result JSONB;
    v_address_count_before INTEGER;
    v_address_count_after INTEGER;
    v_test_address TEXT := '1636 n.w. 114th street, clive, ia, 50325';
BEGIN
    -- Count this specific address before
    SELECT COUNT(*) INTO v_address_count_before
    FROM nodes 
    WHERE normalized_name = normalize_name(v_test_address) AND node_type = 'Address';
    
    -- Test the function - should use existing address
    v_result := propose_geographic_fact(
        'Test Company at Known Address',
        'Company',
        'Clive',
        'City',
        v_test_address,
    ...
    -- Count the address after
    SELECT COUNT(*) INTO v_address_count_after
    FROM nodes 
    WHERE normalized_name = normalize_name(v_test_address) AND node_type = 'Address';
    
    -- Verify no new address was created
    IF v_address_count_after > v_address_count_before THEN
        RAISE NOTICE 'WARNING: New address node was created! Fix may not be working.';
    ELSE
        RAISE NOTICE 'SUCCESS: No new address node created. Using existing entity.';
    END IF;
    
    RAISE NOTICE 'Address count before: %, after: %', v_address_count_before, v_address_count_after;
